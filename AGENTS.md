...
- Use null-safe access `?->` only for genuinely optional object chains; do not hide missing required data.
...
- Use PHP 8.5 `#[NoDiscard]` for immutable methods or functions whose return value must be used.
- Use PHP 8.5 `array_first()` and `array_last()` when retrieving first/last array values; use `array_key_first()` and `array_key_last()` for keys.
- Consider PHP 8.5 URI extension APIs for robust parsing of arbitrary external URLs. Continue using Laravel `route()`, `url()`, signed URLs, validation, and redirect protections for application URLs.
...
- Use progressive enhancement for newer browser features. Provide `@supports` fallbacks when the feature affects layout, interaction, or readability.
- Prefer `oklch()` for new palettes, `color-mix()` for derived states, `text-wrap: balance` for short headings, dynamic viewport units (`svh`, `lvh`, `dvh`) where mobile viewport chrome matters, and container queries for component responsiveness.
...
- Avoid generic AI UI patterns: oversized rounded corners, glassmorphism, decorative blobs, glow-heavy dark SaaS screens, gradient text, fake metrics, ornamental badges, meaningless hero copy, and card grids used as filler.
...
- When styling Filament dashboards, panels, widgets, custom pages, table layouts, stats cards, or dashboard-like admin screens, always activate `uncodixfy` together with `filament-v5-development` and `modern-css-styling` to avoid generic AI dashboard aesthetics.
...
- **Use `Select::make('author_id')->relationship('author', 'name')` for BelongsTo fields.** `BelongsToSelect` does not exist in Filament 5.
